...
- **Tenant is `consumers`** (personal accounts only). Switching to `common` admits work/school accounts and changes which scopes require admin consent. The `consumers` authority rejects an app registration whose `signInAudience` doesn't include personal Microsoft accounts — `whoami` then throws `invalid_grant` _instantly, before printing a device code_. If a user reports that, the fix is in their Azure app registration (Supported account types + Allow public client flows = Yes), not in this code. See README "Azure setup" steps 2 & 5 and the Troubleshooting `invalid_grant` entry.
